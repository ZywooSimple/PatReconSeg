...
def test(val_loader, model, criterion, criterionDice, args, tb, mode):
    ...
    with torch.no_grad():
      for i,(input, target,label,image_index) in enumerate(val_loader): #,angle
        ...
        pred_label=refine_label(pred_label)#############################################################################################对网络输出标签0-1化
...
if __name__ == '__main__':
    ...
    #define filefolder
    test_datasetfile = args.experiment_path + 'test.txt' #测试文件Index
    args.model_output_path = os.path.join(args.experiment_path ,args.exp ,args.date,'model')#模型保存输出位置
    args.dataroot=os.path.join('/hdd2/zmx',args.exp,'h5py')#存储数据的文件夹，文件夹里包含1-1080标号文件夹，内有h5py数据文件
    log_dir=os.path.join(args.experiment_path ,args.exp ,args.date,args.model_name,'test_log')#测试时tensorboard输出位置
