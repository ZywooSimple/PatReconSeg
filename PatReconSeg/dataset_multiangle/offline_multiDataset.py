...
import random 
#这里生成DRR所使用的的CT数据是/hdd/1/zmx/Case10Pack_online包里的
class offline_multiDataSet(Dataset):
  def __init__(self, opt, datasetfile):
    super(offline_multiDataSet, self).__init__()
    self.exp = opt.exp
    self.dataroot = opt.dataroot  ##存储数据的文件夹，文件夹里包含1-1080标号文件夹，内有h5py数据文件
  ...
  def __getitem__(self, item):
    ...
    #生成任意角度DRR
    rtkct_filepath=os.path.join('/hdd2/zmx/'+self.exp+'/', 'ct_rtk_p', str(self.dataset_paths[item])+'.mha')
    geometry_filepath=os.path.join('/hdd2/zmx/1722geometry', '1722geometry.xml')
    ...
    '''
    os.system(f'/home/zmx/RTK/rtksimulatedgeometry -f {str(first_angle)} -n 1 --arc {str(arc)} -o {geometry_filepath}')
    os.system(f'/home/zmx/RTK/rtkforwardprojections -g {geometry_filepath} -i {rtkct_filepath} -o {drr_filepath} --dimension=768 --fp=CudaRayCast')
    '''
